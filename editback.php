...
$Erorr_Flag_name=true;
    ...
    elseif(!preg_match('/^[_\s[:alpha:]]+$/',$name)&&(!preg_match("/\p{Arabic}/u",$name))){
     ...
     if(empty($L_name)){
        $Erorr_array['l_name']="لا يمكن ترك الأسم فارغ";
...
$Erorr_Flag_l_name=true;
    ...
    elseif(!preg_match('/^[_\s[:alpha:]]+$/',$L_name)&&(!preg_match("/\p{Arabic}/u",$L_name))){
        $Erorr_array['l_name']='يجب ان يحتوي الاسم فقط على حروف';
        $Erorr_Flag_l_name=true;
    }
    //validation email
    if(empty($email)){
...
//$tc= isset($_POST[$_SESSION['types']]) ? $_POST[$_SESSION['types']] : "";
    ...
    if ($check&&$Erorr_Flag_name==false&& $Erorr_Flag_l_name==false&&$Erorr_Flag_email==false&&$Erorr_Flag_old_pass==false&&$Erorr_Flag_new_pass==false&&$Erorr_Flag_re_pass==false&&$Erorr_Flag_phone==false){
             if(isset($_POST['sub'])){
                 if($tpp =='Personal'){
                    $query= "update personal_users_table SET First_Name='$name',Last_Name='$L_name',Email='$email',Password='$new_pass',Phone_Number='$phone',Country='$select' where PR_ID='$id_per' ";
                    $Result=mysqli_query($conn,$query);
                 }
else{
    $query= "update company_users_table SET First_Name='$fullname',Email='$email',Password='$new_pass',Phone_Number='$phone'";
    $Result=mysqli_query($conn,$query);
}
             }
    }
